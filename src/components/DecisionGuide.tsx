...
const DecisionGuide: React.FC = () => {
  ...
  // Estado para mostrar mensaje de error de validación
  ...
  useEffect(() => {
    try {
      const savedState = localStorage.getItem('decisionGuideState');
      if (savedState) {
        const parsedState = JSON.parse(savedState);
        if (parsedState && typeof parsedState === 'object') {
          if (Array.isArray(parsedState.answers) && typeof parsedState.currentStep === 'number') {
            setRespuestas(parsedState.answers);
            setPaso(parsedState.currentStep);
            
            // Recalcular puntajes basados en las respuestas guardadas
            const newPuntajes = { redElectrica: 0, panelesSolares: 0, generadorGas: 0 };
            parsedState.answers.forEach((respuesta: number | undefined, index: number) => {
              if (index < preguntas.length && respuesta !== undefined) {
                const puntajeOpcion = preguntas[index].opciones[respuesta]?.puntaje;
                if (puntajeOpcion) {
                  newPuntajes.redElectrica += puntajeOpcion.redElectrica;
                  newPuntajes.panelesSolares += puntajeOpcion.panelesSolares;
                  newPuntajes.generadorGas += puntajeOpcion.generadorGas;
                }
              }
            });
            setPuntajes(newPuntajes);
            
            setAnnouncement(`Estado cargado. Continuando desde la pregunta ${parsedState.currentStep}`);
          }
        }
      }
    } catch (error) {
  ...
  // Efecto para guardar el estado en localStorage cuando cambia
  useEffect(() => {
    if (paso > 0) {
      try {
        localStorage.setItem('decisionGuideState', JSON.stringify({
          currentStep: paso,
          answers: respuestas
        }));
      } catch (error) {
      ...
      // Si volvemos al inicio, limpiar el estado guardado
      localStorage.removeItem('decisionGuideState');
    }
  }, [paso, respuestas]);
  ...
  useEffect(() => {
    ...
    // Limpiar mensaje de error al cambiar de paso
  ...
  }, [actualizarPuntajes, errorMensaje]);
        ...
        <CardContent className="p-6">
          {paso === 0 ? (
            ...
            >
              ...
              <fieldset className="space-y-4" role="radiogroup" aria-label="Opciones de respuesta">
              ...
              <div className="flex justify-between mt-8">
                ...
                <div className="flex flex-col items-end">
                  <button
                    onClick={() => {
                      if (respuestas[paso - 1] !== undefined) {
                        ...
                        // Mostrar mensaje de error cuando no hay selección
                        setErrorMensaje('Por favor seleccione una opción');
                        setAnnouncement('Error: Por favor seleccione una opción antes de continuar');
                      }
                    }}
                  ...
                  {errorMensaje && (
                    <div className="text-red-500 mt-2" data-testid="error-mensaje" aria-live="polite">
                      {errorMensaje}
                    </div>
                  )}
                </div>
              ...
              <div
                ref={resultadosRef}
                tabIndex={0}
                data-testid="resultados-evaluacion"
                className="space-y-8"
              >
              ...
              <div className="bg-blue-50 dark:bg-blue-900/30 p-6 rounded-lg mb-8 text-center">
                ...
                <p className="mt-4 text-blue-800 dark:text-blue-200" data-testid="recomendacion-principal">
                  <strong>Recomendación principal:</strong> {recomendaciones[mejorOpcion.nombre]?.titulo}
                </p>
                <p className="mt-2 text-blue-700 dark:text-blue-300" data-testid="alternativas-sugeridas">
                  <strong>Alternativas sugeridas:</strong> {datosBarras
                    .filter(item => item.nombre !== recomendaciones[mejorOpcion.nombre]?.titulo)
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div>
                  <h4 className="text-xl font-bold mb-4 text-gray-900 dark:text-white" data-testid="analisis-categorias">
                  ...
                  {/* Descripción textual accesible del gráfico */}
                  <div className="sr-only">
                    <h5>Análisis detallado por categorías</h5>
              ...
              <div className="flex space-x-4">
                <button
                  ...
                  data-testid="reiniciar-asistente"
                >
                  Comenzar de Nuevo
                </button>
